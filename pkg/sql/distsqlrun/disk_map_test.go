...
func TestRocksDBMapSandbox(t *testing.T) {
	...
	for i := 0; i < len(diskMaps)-1; i++ {
		diskMaps[i] = NewRocksDBMap(uint64(i) /* prefix */, r)
	}
	// Create a diskMap with a math.MaxUint64 prefix to test the edge case of
	// deletion where there is no end key.
	diskMaps[len(diskMaps)-1] = NewRocksDBMap(math.MaxUint64 /* prefix */, r)
	...
	t.Run("KeyspaceSandbox", func(t *testing.T) {
		for j := 0; j < len(diskMaps); j++ {
			...
			for i.Rewind(); i.Valid(); i.Next() {
				...
				if int(i.Value()[0]) != j {
					t.Fatalf("key %s in %d's keyspace was clobbered by %d", i.Key(), j, i.Value()[0])
				}
			}
			if numRead < numKeys {
	...
	t.Run("KeyspaceDelete", func(t *testing.T) {
		for j := 0; j < len(diskMaps); j++ {
			...
			if numKeysRemaining != expectedKeysRemaining {
				t.Fatalf("expected %d keys to remain but counted %d", expectedKeysRemaining, numKeysRemaining)
			}
		}
	})
}
